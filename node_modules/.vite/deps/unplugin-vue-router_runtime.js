import {
  useRoute,
  useRouter
} from "./chunk-VHH4GE35.js";
import "./chunk-PCSXW7NC.js";
import {
  effectScope,
  ref
...
function isCacheExpired(entry, options) {
  ...
  return !cacheTime || Date.now() - entry.when >= cacheTime || Array.from(entry.loaders).some((childEntry) => isCacheExpired(childEntry, options));
}
...
function shouldFetchAgain(entry, route) {
  return !entry.when || !includesParams(route.params, entry.params) || !includesParams(route.query, entry.query) || entry.hash != null && entry.hash !== route.hash || Array.from(entry.loaders).some((childEntry) => shouldFetchAgain(childEntry, route));
}
...
function setupDataFetchingGuard(router) {
  if (added) {
    console.warn("[vue-router]: Data fetching guard added twice. Make sure to remove the extra call.");
    return;
  }
  added = true;
  return router.beforeEach((to) => {
    return Promise.all(to.matched.flatMap((route) => route.meta[LoaderSymbol]).filter((moduleImport) => moduleImport).map((moduleImport) => moduleImport().then((mod) => {
